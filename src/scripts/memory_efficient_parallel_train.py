...
class MemoryEfficientConfig:
    ...
    n_workers: int = 64  # Reduced for stability
    batch_size: int = 64  # Smaller batch size
    buffer_size: int = 500000  # Smaller buffer
    ...
    experience_chunk_size: int = 500


class SimpleCPUAgent:
    """Lightweight CPU-only agent for workers."""
    
    def __init__(self, map_size: int):
        self.map_size = map_size
        self.epsilon = 1.0
        # No neural network - just heuristic-based agent
        
    def act(self, state, env, valid_mask=None, epsilon=None):
        ...
        if np.random.random() < epsilon:
            ...
            # Use heuristic: target high-value clusters
...
class MemoryEfficientWorker:
    """Memory-efficient CPU-only worker."""
    
    def __init__(self, worker_id: int, config: MemoryEfficientConfig):
        self.worker_id = worker_id
        self.config = config
        self.env = AdvancedCirclePlacementEnv(map_size=config.map_size)
        self.agent = SimpleCPUAgent(config.map_size)
        
    def run_episode(self, epsilon: float) -> Tuple[List, float, float]:
        ...
        while True:
            ...
            action = self.agent.act(state, self.env, valid_mask, epsilon)
...
def memory_efficient_worker_process(worker_id: int, config: MemoryEfficientConfig,
                                  epsilon_queue: mp.Queue, experience_queue: mp.Queue,
                                  control_queue: mp.Queue):
    """Memory-efficient worker process."""
    worker = MemoryEfficientWorker(worker_id, config)
    ...
    while True:
        try:
            ...
            # Get current epsilon
            try:
                epsilon = epsilon_queue.get(timeout=0.1)
            except:
                epsilon = 0.5  # Default epsilon
            
            # Run episode
            experiences, reward, coverage = worker.run_episode(epsilon)
...
class MemoryEfficientReplayBuffer:
    """Simple, memory-efficient replay buffer."""
...
class GPUTrainer:
    """Separate GPU trainer that only handles neural network operations."""
    
    def __init__(self, config: MemoryEfficientConfig):
        self.config = config
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        
        print(f"GPU Trainer using device: {self.device}")
        
        # Create network
        from src.algorithms.dqn_agent import SmartCirclePlacementNet, GuidedDQNAgent
        
        # Create a minimal agent just for the network
        self.temp_agent = GuidedDQNAgent(
            map_size=config.map_size,
            learning_rate=config.learning_rate,
            batch_size=config.batch_size,
            buffer_size=1000,  # Minimal buffer
        )
        
        self.q_network = self.temp_agent.q_network
        self.target_network = self.temp_agent.target_network
        self.optimizer = self.temp_agent.optimizer
        self.gamma = self.temp_agent.gamma
        self.tau = self.temp_agent.tau
        
        # Clear the temp agent to save memory
        del self.temp_agent
    ...
    def train_step(self, batch):
        ...
        try:
            ...
            # Prepare state batch
            current_maps = torch.FloatTensor([s["current_map"] for s in states]).to(self.device)
            placed_masks = torch.FloatTensor([s["placed_mask"] for s in states]).to(self.device)
            value_densities = torch.FloatTensor([s["value_density"] for s in states]).to(self.device)
            features = torch.FloatTensor([s["features"] for s in states]).to(self.device)
            ...
            if next_states:
                next_current_maps = torch.FloatTensor([s["current_map"] for s in next_states]).to(self.device)
                next_placed_masks = torch.FloatTensor([s["placed_mask"] for s in next_states]).to(self.device)
                next_value_densities = torch.FloatTensor([s["value_density"] for s in next_states]).to(self.device)
                next_features = torch.FloatTensor([s["features"] for s in next_states]).to(self.device)
                ...
                with torch.no_grad():
                    # Double DQN
                    next_actions = self.q_network(next_state_batch).view(len(next_states), -1).max(1)[1]
...
class MemoryEfficientTrainer:
    """Memory-efficient parallel trainer."""
    
    def __init__(self, config: MemoryEfficientConfig):
        self.config = config
        
        # GPU trainer (separate process would be even better, but this works)
        self.gpu_trainer = GPUTrainer(config)
        
        # Replay buffer
        self.replay_buffer = MemoryEfficientReplayBuffer(config.buffer_size)
        
        # Multiprocessing setup
        self.epsilon_queues = [mp.Queue(maxsize=2) for _ in range(config.n_workers)]
        ...
        for i in range(config.n_workers):
            worker = mp.Process(
                target=memory_efficient_worker_process,
                args=(i, config, self.epsilon_queues[i], self.experience_queue, self.control_queues[i])
            )
            worker.start()
    ...
    def _distribute_epsilon(self):
        """Distribute current epsilon to workers."""
        for q in self.epsilon_queues:
            try:
                if q.empty():
                    q.put_nowait(self.epsilon)
            except:
                pass
    
    def visualize_strategy(self, episode, save_path="memory_efficient_strategy.png"):
        ...
        for i in range(len(test_env.radii)):
            ...
            # Get agent's action using GPU network
            with torch.no_grad():
                current_map = torch.FloatTensor(state["current_map"]).unsqueeze(0).to(self.gpu_trainer.device)
                placed_mask = torch.FloatTensor(state["placed_mask"]).unsqueeze(0).to(self.gpu_trainer.device)
                value_density = torch.FloatTensor(state["value_density"]).unsqueeze(0).to(self.gpu_trainer.device)
                features = torch.FloatTensor(state["features"]).unsqueeze(0).to(self.gpu_trainer.device)
                ...
                q_values = self.gpu_trainer.q_network(state_batch).squeeze(0)
                
                if valid_mask is not None:
                    mask_tensor = torch.FloatTensor(valid_mask).to(self.gpu_trainer.device)
        ...
        axes[1, 2].text(0.1, 0.6, f"Training Steps: {self.gpu_trainer.training_step}", fontsize=12, transform=axes[1, 2].transAxes)
        ...
        plt.suptitle(f"Memory-Efficient Agent Strategy - Episode {episode}")
        plt.tight_layout()
    ...
    def train(self):
        ...
        print("=" * 100)
        print(f"MEMORY-EFFICIENT PARALLEL TRAINING WITH {self.config.n_workers} WORKERS")
        print("=" * 100)
        ...
        pbar = tqdm(total=self.config.n_episodes, desc="Memory-Efficient Training")
        ...
        while len(self.episode_rewards) < self.config.n_episodes:
            ...
            # Distribute epsilon to workers
            self._distribute_epsilon()
            ...
            if len(self.replay_buffer) > self.config.batch_size * self.config.gradient_accumulation_steps:
                ...
                for _ in range(self.config.gradient_accumulation_steps):
                    ...
                    if batch is not None:
                        loss = self.gpu_trainer.train_step(batch)
                        if loss is not None:
                            total_loss += loss
                            num_batches += 1
        ...
        print(f"Total training steps: {self.gpu_trainer.training_step:,}")
    ...
    def _evaluate_and_visualize(self, episode: int):
        ...
        print(f"  Training steps: {self.gpu_trainer.training_step:,}")
        ...
        # Visualize strategy
        self.visualize_strategy(episode, f"memory_efficient_strategy_ep{episode}.png")
        print(f"\nStrategy visualization saved!")
        
        # Save checkpoint
        if episode % self.config.save_every == 0:
            torch.save({
                "model_state_dict": self.gpu_trainer.q_network.state_dict(),
                "target_state_dict": self.gpu_trainer.target_network.state_dict(),
                "episode": episode,
                "training_step": self.gpu_trainer.training_step,
                "coverage_stats": {
                    "recent": recent_coverage,
                    "best": best_coverage,
                    "std": np.std(self.episode_coverage[-1000:])
                }
            }, f"memory_efficient_model_ep{episode}.pth")
            print(f"Checkpoint saved: memory_efficient_model_ep{episode}.pth")
    ...
    def _save_final_model(self):
        ...
        torch.save({
            "model_state_dict": self.gpu_trainer.q_network.state_dict(),
            "target_state_dict": self.gpu_trainer.target_network.state_dict(),
            "optimizer_state_dict": self.gpu_trainer.optimizer.state_dict(),
            ...
            "losses": self.losses,
            "config": self.config,
        }, "memory_efficient_final_model.pth")
    ...
    def _plot_training_progress(self):
        ...
        plt.suptitle(f'Memory-Efficient Training Results - {len(self.episode_rewards):,} Episodes')
        plt.tight_layout()
        plt.savefig('memory_efficient_training_results.png', dpi=150, bbox_inches='tight')
        plt.show()
...
def main():
    ...
    n_workers = min(64, max(16, n_cores - 16))  # Leave plenty of cores for system
    
    print(f"System: {n_cores} cores")
    print(f"Using {n_workers} workers for memory-efficient training")
    
    config = MemoryEfficientConfig(
        n_episodes=100000,
        n_workers=n_workers,
        batch_size=64,  # Conservative batch size
        buffer_size=500000,  # Conservative buffer size
        learning_rate=1e-4,

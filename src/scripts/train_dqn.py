    ...
    for i in range(len(env.radii)):
        ...
        if i + 1 in steps_to_show and step_idx < 3:
            ...
            axes[row, col].set_title(
                f"After {i + 1} circles (Coverage: {info['coverage']:.1%})"
            )
...
def train_advanced_agent(n_episodes=2000, visualize_every=200):
    ...
    for episode in pbar:
        ...
        if (episode + 1) % visualize_every == 0:
            ...
            # Visualize strategy
            visualize_strategy(
                env, agent, episode + 1, f"advanced_strategy_ep{episode + 1}.png"
            )
            ...
            test_coverages = []
            for i in range(5):
                test_map = random_seeder(MAP_SIZE, time_steps=100000)
                test_env = AdvancedCirclePlacementEnv(MAP_SIZE)
    ...
    ax1.plot(
        np.convolve(episode_coverage, np.ones(100) / 100, mode="valid"),
...
if __name__ == "__main__":
    train_advanced_agent(n_episodes=50000, visualize_every=200)

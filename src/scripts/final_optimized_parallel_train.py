...
class FinalOptimizedConfig:
    ...
    n_workers: int = 32  # Conservative for stability
    batch_size: int = 64
    ...
    epsilon_decay_episodes: int = 20000  # Decay over 20k episodes instead of 100k


class OptimizedHeuristicAgent:
    """Optimized heuristic agent for workers."""
    
    def __init__(self, map_size: int):
        self.map_size = map_size
        
    def act(self, state_dict, env, valid_mask=None, epsilon=0.5):
        """Choose action using optimized heuristics."""
    ...
    while True:
        try:
            ...
            while True:
                ...
                # Efficiently convert states to numpy arrays (not lists!)
...
class FinalOptimizedTrainer:
    """Final optimized parallel trainer with all fixes."""
    ...
    def __init__(self, config: FinalOptimizedConfig):
        ...
        # Fixed mixed precision with new API
        ...
        # Shared epsilon with proper initialization
    ...
    def _update_epsilon(self):
        ...
        # Linear decay over specified episodes
    ...
    def _prepare_batch_tensors_optimized(self, batch):
        """Optimized tensor preparation - fixes the performance warning!"""
        ...
        # OPTIMIZED: Convert to single numpy array first, then to tensor
        ...
        # Now convert numpy arrays to tensors (much faster!)
    ...
    def _train_step(self):
        """Optimized training step with fixed autocast."""
        batch = self.replay_buffer.sample(self.config.batch_size)
        if batch is None:
            return None
        
        try:
            # Use optimized batch preparation
            ...
            # Convert actions to indices efficiently
            ...
            if next_states:
                # Optimized next state processing
            ...
            # Backward pass with fixed mixed precision
    ...
    def visualize_strategy(self, episode, save_path="final_optimized_strategy.png"):
        ...
        plt.suptitle(f"Final Optimized Agent Strategy - Episode {episode}")
        plt.tight_layout()
    ...
    def train(self):
        ...
        print("=" * 100)
        print(f"FINAL OPTIMIZED PARALLEL TRAINING WITH {self.config.n_workers} WORKERS")
        print("=" * 100)
        ...
        pbar = tqdm(total=self.config.n_episodes, desc="Final Optimized Training")
        ...
        while len(self.episode_rewards) < self.config.n_episodes:
            ...
            # Update epsilon with proper decay
            ...
            # Fixed periodic evaluation - ensure it always triggers
    ...
    def _evaluate_and_visualize(self, episode: int):
        """Fixed evaluation and visualization - always executes."""
        print(f"\n{'=' * 80}")
        ...
        try:
            self.visualize_strategy(episode, f"final_optimized_strategy_ep{episode}.png")
        ...
        if episode % self.config.save_every == 0:
            torch.save({
                ...
                "coverage_stats": {
                    ...
                    "best": best_coverage,
                    "std": np.std(self.episode_coverage[-1000:]) if len(self.episode_coverage) >= 1000 else 0
                }
            }, f"final_optimized_model_ep{episode}.pth")
            print(f"Checkpoint saved: final_optimized_model_ep{episode}.pth")
    ...
    def _plot_training_progress(self):
        ...
        epsilons = []
        for ep in episodes:
            if ep < self.config.epsilon_decay_episodes:
                decay_progress = ep / self.config.epsilon_decay_episodes
                epsilon = self.config.epsilon_start - (self.config.epsilon_start - self.config.epsilon_end) * decay_progress
            else:
                epsilon = self.config.epsilon_end
            epsilons.append(epsilon)
        
        ax4.plot(episodes, epsilons, linewidth=2, color='green')
        ax4.set_title('Epsilon Decay')
        ax4.set_xlabel('Episode')
        ax4.set_ylabel('Epsilon')
        ax4.grid(True)
        
        plt.suptitle(f'Final Optimized Training Results - {len(self.episode_rewards):,} Episodes')
        plt.tight_layout()
        plt.savefig('final_optimized_training_results.png', dpi=150, bbox_inches='tight')
        plt.show()
...
def main():
    """Main final optimized training function."""
    try:
        n_cores = mp.cpu_count()
    except:
        n_cores = 32
    
    # Optimized worker count for your 128-core system
    n_workers = min(64, max(16, n_cores - 8))
    
    print(f"System: {n_cores} cores")
    print(f"Using {n_workers} workers for final optimized training")
    ...
    config = FinalOptimizedConfig(
        ...
        epsilon_start=1.0,
        epsilon_end=0.01,
        epsilon_decay_episodes=20000,  # Much faster decay
    )

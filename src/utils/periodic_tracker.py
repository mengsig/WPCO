...
class PeriodicTaskTracker:
    ...
    def register_task(self, name: str, interval: int, callback: Callable, 
                     start_step: int = 0, enabled: bool = True):
        """
            ...
            for name, task in self.tasks.items():
                ...
                if current_step >= task['next_target']:
                    try:
                        ...
                        missed_intervals = (current_step - task['next_target']) // task['interval']
    ...
    def reset_task(self, name: str, current_step: int):
        ...
        with self.lock:
            if name in self.tasks:
                self.tasks[name]['last_executed'] = current_step
                self.tasks[name]['next_target'] = current_step + self.tasks[name]['interval']

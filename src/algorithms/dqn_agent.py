...
class AdvancedCirclePlacementEnv:
    ...
    def _get_enhanced_state(self):
        ...
        state_dict = {
            ...
            "total_remaining_area": total_remaining_area / (self.map_size * self.map_size),
    ...
    def _encode_features(self, features, radius):
        ...
        base_features = 10
        total_features = base_features + max_radii * 2  # radii values + placement status
    ...
    def analyze_future_placement_potential(self, x, y, current_radius):
        ...
        future_placement_ratio = future_fit_count / max(1, len(self.radii) - self.current_radius_idx - 1)
    ...
    def visualize_radii_context(self):
        ...
        print(f"Current progress: {self.current_radius_idx}/{len(self.radii)} ({self.current_radius_idx/len(self.radii)*100:.1f}%)")
        
        print("\nRadii sequence:")
        for i, r in enumerate(self.radii):
            status = "✓" if i < self.current_radius_idx else ("→" if i == self.current_radius_idx else " ")
            area = np.pi * r * r
        ...
        print(f"Coverage percentage: {self.total_weight_collected/self.original_map.sum()*100:.1f}%")
        print("==================================\n")

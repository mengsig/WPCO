...
class AdvancedCirclePlacementEnv:
    ...
    def step(self, action):
        ...
        if included_weight > 0:
            reward = included_weight / (np.pi * radius * radius * self.original_map.max())
        ...
        for cluster in features['clusters']:
            ...
            dist_to_cluster = np.sqrt((x - cluster_center[0])**2 + (y - cluster_center[1])**2)
    ...
    def get_valid_actions_mask(self):
        ...
        if np.any(mask & (self.current_map > self.current_map.max() * 0.3)):
            ...
            value_bonus = (self.current_map > self.current_map.max() * 0.3).astype(float)
...
def compute_included(weighted_matrix, x, y, radius):
    included_weight = 0.0
    for i in range(max(0, int(x - radius)), min(weighted_matrix.shape[0], int(x + radius + 1))):
        for j in range(max(0, int(y - radius)), min(weighted_matrix.shape[1], int(y + radius + 1))):
            if (i - x) ** 2 + (j - y) ** 2 <= radius ** 2:
...
class GuidedDQNAgent:
    ...
    def __init__(self, map_size=64, learning_rate=1e-4, gamma=0.99,
                 epsilon_start=0.5, epsilon_end=0.01, epsilon_decay_steps=2000,
                 batch_size=32, buffer_size=50000, tau=0.001,
                 use_suggestions=True, suggestion_prob=0.3):
    ...
    def act(self, state_dict, env, valid_mask=None):
        ...
        if self.use_suggestions and random.random() < self.suggestion_prob * self.epsilon:
        ...
        with torch.no_grad():
            ...
            if valid_mask is not None:
                ...
                q_values = q_values + (mask_tensor - 1) * 1e10  # Large negative for invalid
    ...
    def _prepare_state_batch(self, state_dicts):
        ...
        features = torch.FloatTensor(
            np.array([s['features'] for s in state_dicts])
        ).to(self.device)
    ...
    def replay(self):
        ...
        current_q_values = current_q_values.view(self.batch_size, -1).gather(1, action_indices).squeeze()
        ...
        if next_states:
            ...
            with torch.no_grad():
                ...
                next_q_values[non_final_mask] = self.target_network(
                    next_state_batch
                ).view(len(next_states), -1).gather(1, next_actions.unsqueeze(1)).squeeze()
